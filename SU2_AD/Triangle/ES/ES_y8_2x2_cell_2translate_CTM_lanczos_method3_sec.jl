...
@tensor TL7[:]:=TL7[-2,1,-4]*U_L1[1,-3,-1]; #R,D,R',U
@tensor TL8[:]:=TL8[-2,1,-4]*U_L2[1,-3,-1]; #R,D,R',U
...
@tensor TR7[:]:=TR7[-4,1,-2]*U_L1'[-1,-3,1]; #L',D,L',U
@tensor TR8[:]:=TR8[-4,1,-2]*U_L2'[-1,-3,1]; #L',D,L',U
...
@assert norm(U_D_D1a-U_D_D3a)<1e-10;#ensure the same virtual space, otherwise need to be very careful

U_D_D1b=unitary(fuse(space(TL3,1)*space(TL2,1)), space(TL3,1)*space(TL2,1));

function vr_ML_MR(vr0,  TL_set,TR_set, gate_middle_set)
    println("apply Mr");flush(stdout);
    global U_D_D1a,U_D_D2a,U_D_D3a,y_anti_pbc
    ################
    if y_anti_pbc
    ...
    @tensor vr0[:]:=vr0[7,4,1,3,-5,-6]*U_D_D1a'[-1,8,7]*U_D_D1a'[9,5,4]*U_D_D1a'[6,2,1]*U_D_D1b[-2,8,9]*U_D_D1b[-3,5,6]*U_D_D1b[-4,2,3];
    ################
    if y_anti_pbc
        op=parity_gate(vr0,1);
        @tensor vr0[:]:=op[-1,1]*vr0[1,-2,-3,-4,-5,-6];
    ...
    vr0=permute_neighbour_ind(vr0,3,4,6);
    vr0=permute_neighbour_ind(vr0,4,5,6);
    @tensor vr0[:]:=vr0[7,4,1,3,-5,-6]*U_D_D1b'[-1,8,7]*U_D_D1b'[9,5,4]*U_D_D1b'[6,2,1]*U_D_D1a[-2,8,9]*U_D_D1a[-3,5,6]*U_D_D1a[-4,2,3];
    ################
...
for ss=S_ind:S_ind#1:length(Spin_set)
    ...
    #############################
    if y_anti_pbc
        matnm="ES_CTM_2translate_D"*string(D)*"_chi"*string(chi)*"_Nv8_APBC_S"*string(Spin_set[ss])*".mat";
    else
        matnm="ES_CTM_2translate_D"*string(D)*"_chi"*string(chi)*"_Nv8_PBC_S"*string(Spin_set[ss])*".mat";
    end
    matwrite(matnm, Dict(
        "ks" => ks,
        "eu0" => eu0,
