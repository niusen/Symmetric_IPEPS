...
@tensor ML[:]:=ML[1,2,3,4]*U_L[1,-1,-2]*U_L[2,-3,-4]*U_L[3,-5,-6]*U_L[4,-7,-8];#R1',R1,R2',R2,R3',R3,R4',R4
ML=permute(ML,(2,1,4,3,6,5,8,7,));#R1,R1',R2,R2',R3,R3',R4,R4'
...
@tensor ML_temp[:]:=ML[-1,2,3,-4,-5,-6,-7,-8]*P_odda[-3,3]*P_oddb[-2,2];#R2,R1'
ML=ML-2*ML_temp;
@tensor ML_temp[:]:=ML[-1,2,-3,-4,5,-6,-7,-8]*P_odda[-5,5]*P_oddb[-2,2];#R3,R1'
ML=ML-2*ML_temp;
@tensor ML_temp[:]:=ML[-1,2,-3,-4,-5,-6,7,-8]*P_odda[-7,7]*P_oddb[-2,2];#R4,R1'
ML=ML-2*ML_temp;

@tensor ML_temp[:]:=ML[-1,-2,-3,4,5,-6,-7,-8]*P_odda[-5,5]*P_oddb[-4,4];#R3,R2'
ML=ML-2*ML_temp;
@tensor ML_temp[:]:=ML[-1,-2,-3,4,-5,-6,7,-8]*P_odda[-7,7]*P_oddb[-4,4];#R4,R2'
ML=ML-2*ML_temp;

@tensor ML_temp[:]:=ML[-1,-2,-3,-4,-5,6,7,-8]*P_odda[-7,7]*P_oddb[-6,6];#R4,R3'
ML=ML-2*ML_temp;
...
P_odda=P_odda'*P_odda;
P_oddb=P_oddb'*P_oddb;
@tensor MR_temp[:]:=MR[-1,-2,-3,-4,-5,6,7,-8]*P_odda[-6,6]*P_oddb[-7,7];#L2,L1'
MR=MR-2*MR_temp;
@tensor MR_temp[:]:=MR[-1,-2,-3,4,-5,-6,7,-8]*P_odda[-4,4]*P_oddb[-7,7];#L3,L1'
MR=MR-2*MR_temp;
@tensor MR_temp[:]:=MR[-1,2,-3,-4,-5,-6,7,-8]*P_odda[-2,2]*P_oddb[-7,7];#L4,L1'
MR=MR-2*MR_temp;

@tensor MR_temp[:]:=MR[-1,-2,-3,4,5,-6,-7,-8]*P_odda[-4,4]*P_oddb[-5,5];#L3,L2'
MR=MR-2*MR_temp;
@tensor MR_temp[:]:=MR[-1,2,-3,-4,5,-6,-7,-8]*P_odda[-2,2]*P_oddb[-5,5];#L4,L2'
MR=MR-2*MR_temp;

@tensor MR_temp[:]:=MR[-1,2,3,-4,-5,-6,-7,-8]*P_odda[-2,2]*P_oddb[-3,3];#L4,L3'
MR=MR-2*MR_temp;
...
@tensor H[:]:=MR[-4,4,-3,3,-2,2,-1,1]*ML[1,-5,2,-6,3,-7,4,-8];#L6',L6,L5',L5,L4',L4,L3',L3,L2',L2,L1',L1,     R1,R1',R2,R2',R3,R3',R4,R4',R5,R5',R6,R6'   
H=permute(H,(1,2,3,4,),(5,6,7,8,));#L1',L2',L3',L4',L5',L6',   R1',R2',R3',R4',R5',R6'
...
if y_anti_pbc
    ev=permute(ev,(1,2,3,4,5,));#L1',L2',L3',L4',L5',L6',dumm
    op=parity_gate(ev,4);
