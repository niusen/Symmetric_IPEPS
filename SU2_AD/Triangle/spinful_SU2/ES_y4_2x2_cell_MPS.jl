...
A3=permute(A3,(1,4,5,3,2,));#LUPRD
A4=permute(A4,(1,4,5,3,2,));#LUPRD

B1=permute(B1,(1,4,5,3,2,));#LUPRD
B2=permute(B2,(1,4,5,3,2,));#LUPRD
B3=permute(B3,(1,4,5,3,2,));#LUPRD
B4=permute(B4,(1,4,5,3,2,));#LUPRD
...
B123,UL123b,UR123b,Up123b=combine_mps_2row(B12,B3);
B1234,UL1234b,UR1234b,Up1234b=combine_mps_2row(B123,B4);
B1234=permute_neighbour_ind(B1234,2,3,5);#L P U R D 
B1234=permute_neighbour_ind(B1234,3,4,5);#L P R U D 
B1234=permute_neighbour_ind(B1234,4,5,5);#L P R D U 
@tensor B1234[:]:=B1234[-1,-2,-3,1,1];#L P R
#############################
...
v_init=TensorMap(randn, space(A1234,1)',space(A1234,1)');
v_init=permute(v_init,(1,2,),());#L1,L2,L3,L4,dummy
contraction_fun_R(x)=M_vr(x,A1234',B1234',A1234,B1234);
@time eur,evr=eigsolve(contraction_fun_R, v_init, 1,:LM,Arnoldi(krylovdim=40));
VR=evr[findmax(abs.(eur))[2]];#L1,L2,L3,L4,dummy


v_init=TensorMap(randn, space(B1234,3)',space(B1234,3)');
v_init=permute(v_init,(1,2,),());#dummy,R1,R2,R3,R4
contraction_fun_L(x)=vl_M(x, A1234',B1234',A1234,B1234);
...
############################
function translate_AAAA(M1234,Up1234,Up123,Up12,y_anti_pbc)
    M1234_translated=deepcopy(M1234);
    @tensor M1234_translated[:]:=M1234_translated[-1,1,-4]*Up1234'[-2,-3,1];#L,P123,P4,R
    @tensor M1234_translated[:]:=M1234_translated[-1,1,-4,-5]*Up123'[-2,-3,1];#L,P12,P3,P4,R
    @tensor M1234_translated[:]:=M1234_translated[-1,1,-4,-5,-6]*Up12'[-2,-3,1];#L,P1,P2,P3,P4,R
    if y_anti_pbc
        op=parity_gate(M1234_translated,2);
        @tensor M1234_translated[:]:=op[-2,1]*M1234_translated[-1,1,-3,-4,-5,-6];
    end
        
    M1234_translated=permute_neighbour_ind(M1234_translated,2,3,6);#L,P2,P1,P3,P4,R
    M1234_translated=permute_neighbour_ind(M1234_translated,3,4,6);#L,P2,P3,P1,P4,R
    M1234_translated=permute_neighbour_ind(M1234_translated,4,5,6);#L,P2,P3,P4,P1,R
    @tensor M1234_translated[:]:=M1234_translated[-1,1,2,-3,-4,-5]*Up12[-2,1,2];
    @tensor M1234_translated[:]:=M1234_translated[-1,1,2,-3,-4]*Up123[-2,1,2];
    @tensor M1234_translated[:]:=M1234_translated[-1,1,2,-3]*Up1234[-2,1,2];
    return M1234_translated
end

A1234_translated=translate_AAAA(A1234,Up1234a,Up123a,Up12a,y_anti_pbc);
B1234_translated=translate_AAAA(B1234,Up1234b,Up123b,Up12b,y_anti_pbc);


v_init=TensorMap(randn, space(A1234,1)',space(A1234,1)');
v_init=permute(v_init,(1,2,),());#L1,L2,L3,L4,dummy
contraction_fun_R(x)=M_vr(x,A1234_translated',B1234_translated',A1234,B1234);
...
v_init=TensorMap(randn, space(B1234,3)',space(B1234,3)');
v_init=permute(v_init,(1,2,),());#dummy,R1,R2,R3,R4
contraction_fun_L(x)=vl_M(x, A1234_translated',B1234_translated',A1234,B1234);
