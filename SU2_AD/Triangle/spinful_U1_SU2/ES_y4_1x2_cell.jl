...
y_anti_pbc=true;
filenm="Optim_cell_LS_D_4_chi_40_2.3784.jld2";
data=load(filenm);
A=data["x"][1].T;
B=data["x"][2].T;
...
global V_trivial
if isa(space(A,1), GradedSpace{ProductSector{Tuple{U1Irrep, SU2Irrep}}, TensorKit.SortedVectorDict{ProductSector{Tuple{U1Irrep, SU2Irrep}}, Int64}})
    V_trivial=Rep[U₁ × SU₂]((0, 0)=>1);
elseif isa(space(A,1), GradedSpace{SU2Irrep, TensorKit.SortedVectorDict{SU2Irrep, Int64}})
...
function construct_tensors(A1,A2,A3,A4)
    ...
    V_odd,V_even=projector_parity(space(A1,4));
    @assert norm(V_odd'*V_odd+V_even'*V_even-unitary(space(V_even,2)',space(V_even,2)'))<1e-12

    #Phy_odd,Phy_even=projector_physical(space(A1,5));
    Phy_odd,Phy_even=projector_parity(space(A1,5));
    @assert norm(Phy_odd'*Phy_odd+Phy_even'*Phy_even-unitary(space(Phy_even,2)',space(Phy_even,2)'))<1e-12
...
function M_vr(l_Vodd,l_Veven,vr0,  AA1_set,AA2_set,AA3_set,AA4_set,  gate_set)
    ...
    vvv_init=TensorMap(randn, space(AA2_set[1],1)*space(AA2_set[1],1)*space(AA2_set[1],1)*space(AA2_set[1],1),V_trivial);
    vvv_init=permute(vvv_init,(1,2,3,4,5,),());#L1,L2,L3,L4,dummy

    vr=deepcopy(vvv_init)*0;#L1,L2,L3,L4,dummy
    #sign from U1+U1'
            ...
            for parity_P2=0:1
                for parity_P3=0:1
                    for parity_P4=0:1
                        ...
                        gate=parity_gate(AA1_temp,4); 
                        @tensor AA1_temp[:]:=AA1_temp[-1,-2,-3,1]*gate[-4,1];#sign of U1,U1'
                        if parity_P2==1
...
function vl_M(l_Vodd,l_Veven,vl0,  AA1_set,AA2_set,AA3_set,AA4_set,  gate_set)
    ...
    vvv_init=TensorMap(randn, space(AA2_set[1],3)*space(AA2_set[1],3)*space(AA2_set[1],3)*space(AA2_set[1],3),V_trivial');
    vvv_init=permute(vvv_init,(5,1,2,3,4,),());#dummy,R1,R2,R3,R4
    vl=deepcopy(vvv_init)*0;#dummy,R1,R2,R3
    #sign from U1+U1'
...
l_Vodd,l_Veven,AA1_set,AA2_set,AA3_set,AA4_set,gateA_set,U_L1,U_D1,U_R1,U_U1=construct_tensors(A1,A2,A3,A4);
l_Vodd,l_Veven,BB1_set,BB2_set,BB3_set,BB4_set,gateB_set,U_L2,U_D2,U_R2,U_U2=construct_tensors(B1,B2,B3,B4);

v_init=TensorMap(randn, space(AA2_set[1],1)*space(AA2_set[1],1)*space(AA2_set[1],1)*space(AA2_set[1],1),V_trivial);
...
if norm(eur)<1e-8
    error("eigenvalue too small, need to normalize")
end    

v_init=TensorMap(randn, space(BB2_set[1],3)*space(BB2_set[1],3)*space(BB2_set[1],3)*space(BB2_set[1],3),V_trivial');
...
@tensor VL[:]:=VL[-1,1,2,3,4]*U_L1[1,-2,-3]*U_L1[2,-4,-5]*U_L1[3,-6,-7]*U_L1[4,-8,-9];#dummy, R1',R1,R2',R2,R3',R3,R4',R4
VL=permute(VL,(1,2,4,6,8,3,5,7,9,));#dummy, R1',R2',R3',R4', R1,R2,R3,R4


@tensor VR[:]:=VR[1,2,3,4,-9]*U_L1'[-1,-2,1]*U_L1'[-3,-4,2]*U_L1'[-5,-6,3]*U_L1'[-7,-8,4];#L1',L1,L2',L2,L3',L3,L4',L4,dummy
...
Qn=[];
if isa(space(H,1), GradedSpace{TensorKit.ProductSector{Tuple{U1Irrep, SU2Irrep}}, TensorKit.SortedVectorDict{TensorKit.ProductSector{Tuple{U1Irrep, SU2Irrep}}, Int64}})
    Qn=Int64.(get_Vspace_Qn(space(eu,1)));
end
