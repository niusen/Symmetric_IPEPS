function operators_spinless_Z2()
...
function operators_spinless_U1()
...
function prepare_nn_x_LD_RD(O1,O2,A_cell)
    # pos_LU=[1,2];
    # pos_RU=[2,2];
    pos_LD=[1,1];
    pos_RD=[2,1];
...
function prepare_nn_x_LU_RU(O1,O2,A_cell)
...
function prepare_nn_y_RU_RD(O1,O2,A_cell)
    # pos_LU=[1,2];
    pos_RU=[2,2];
    # pos_LD=[1,1];
    pos_RD=[2,1];
...
function prepare_nn_y_LU_LD(O1,O2,A_cell)
    ...
    #the first index of O is dummy
    @tensor A_LU[:]:= A_cell[pos_LU[1],pos_LU[2]][-1,-2,-3,-4,1]*O1[-5,1]
    @tensor A_LD[:]:= A_cell[pos_LD[1],pos_LD[2]][-1,-2,-3,-4,1]*O2[-5,1]
...
function prepare_nn_LD_RU(O1,O2,A_cell)
    # pos_LU=[1,2];
    pos_RU=[2,2];
    pos_LD=[1,1];
    pos_RD=[2,1];
...
function compute_ob_2x2_triangle(mps_bot_set,mps_top_set,iPEPS_2x2_, iPEPS_double_2x2, VL_set_set,VR_set_set, x_range,y_range)
    ...
    elseif isa(space(psi_double[1,1],1),GradedSpace{U1Irrep, TensorKit.SortedVectorDict{U1Irrep, Int64}})
        Hamiltonian_terms=operators_spinless_U1;
    ...
    elseif isa(space(psi_double[1,1],1),GradedSpace{ProductSector{Tuple{U1Irrep, SU2Irrep}}, TensorKit.SortedVectorDict{ProductSector{Tuple{U1Irrep, SU2Irrep}}, Int64}})
        Hamiltonian_terms=operators_spinful_U1_SU2;
    end

    Ident, N_occu, Cdag, C, _ =@ignore_derivatives Hamiltonian_terms();
    ##########################


    Norm_=contract_2x2(VL0,VR0,x_range,y_range,mps_top,mps_bot,iPEPS_double_2x2[1,2],iPEPS_double_2x2[2,2],iPEPS_double_2x2[1,1],iPEPS_double_2x2[2,1]);

    AA_LD,AA_RD=prepare_hopping_x_LD_RD(Cdag,C,iPEPS_2x2);#tx
    ex=contract_2x2(VL0,VR0,x_range,y_range,mps_top,mps_bot,iPEPS_double_2x2[1,2],iPEPS_double_2x2[2,2],AA_LD,AA_RD);
    ex=ex/Norm_;

    AA_LD,AA_RD=prepare_nn_x_LD_RD(N_occu,N_occu,iPEPS_2x2);#tx
    nnx=contract_2x2(VL0,VR0,x_range,y_range,mps_top,mps_bot,iPEPS_double_2x2[1,2],iPEPS_double_2x2[2,2],AA_LD,AA_RD);
    nnx=nnx/Norm_;

    AA_RD,AA_RU=prepare_hopping_y_RU_RD(Cdag,C,iPEPS_2x2);#ty
    ey=contract_2x2(VL0,VR0,x_range,y_range,mps_top,mps_bot,iPEPS_double_2x2[1,2],AA_RU,iPEPS_double_2x2[1,1],AA_RD);
    ey=ey/Norm_;

    AA_RD,AA_RU=prepare_nn_y_RU_RD(N_occu,N_occu,iPEPS_2x2);#ty
    nny=contract_2x2(VL0,VR0,x_range,y_range,mps_top,mps_bot,iPEPS_double_2x2[1,2],AA_RU,iPEPS_double_2x2[1,1],AA_RD);
    nny=nny/Norm_;

    AA_LD, AA_RU, AA_RD=prepare_hopping_LD_RU(Cdag,C,iPEPS_2x2);#t_ld_ru
    e_ld_ru=contract_2x2(VL0,VR0,x_range,y_range,mps_top,mps_bot,iPEPS_double_2x2[1,2],AA_RU,AA_LD,AA_RD);
    e_ld_ru=e_ld_ru/Norm_;

    AA_LD, AA_RU, AA_RD=prepare_nn_LD_RU(N_occu,N_occu,iPEPS_2x2);#t_ld_ru
    nn_ld_ru=contract_2x2(VL0,VR0,x_range,y_range,mps_top,mps_bot,iPEPS_double_2x2[1,2],AA_RU,AA_LD,AA_RD);
    nn_ld_ru=nn_ld_ru/Norm_;

    AA_RD=prepare_RD(N_occu,iPEPS_2x2);#e0
    ...
    return ex,ey,e_ld_ru, nnx,nny,nn_ld_ru, occu
end
...
function compute_ob_2x2_top(mps_bot_set,mps_top_set,iPEPS_2x2_, iPEPS_double_2x2, VL_set_set,VR_set_set, x_range,y_range)
    ...
    AA_LU,AA_RU=prepare_hopping_x_LU_RU(Cdag,C,iPEPS_2x2);#tx
    ex=contract_2x2(VL0,VR0,x_range,y_range,mps_top,mps_bot,AA_LU,AA_RU,iPEPS_double_2x2[1,1],iPEPS_double_2x2[2,1]);
    ex=ex/Norm_;

    AA_LU,AA_RU=prepare_nn_x_LU_RU(N_occu,N_occu,iPEPS_2x2);#tx
    nnx=contract_2x2(VL0,VR0,x_range,y_range,mps_top,mps_bot,AA_LU,AA_RU,iPEPS_double_2x2[1,1],iPEPS_double_2x2[2,1]);
    nnx=nnx/Norm_;

    AA_RU=prepare_RU(N_occu,iPEPS_2x2);#e0
...
function compute_ob_2x2_left(mps_bot_set,mps_top_set,iPEPS_2x2_, iPEPS_double_2x2, VL_set_set,VR_set_set, x_range,y_range)
    ...
    AA_LD,AA_LU=prepare_hopping_y_LU_LD(Cdag,C,iPEPS_2x2);#ty
    ey=contract_2x2(VL0,VR0,x_range,y_range,mps_top,mps_bot,AA_LU,iPEPS_double_2x2[2,2],AA_LD,iPEPS_double_2x2[2,1]);
    ey=ey/Norm_;

    AA_LD,AA_LU=prepare_nn_y_LU_LD(N_occu,N_occu,iPEPS_2x2);#ty
    nny=contract_2x2(VL0,VR0,x_range,y_range,mps_top,mps_bot,AA_LU,iPEPS_double_2x2[2,2],AA_LD,iPEPS_double_2x2[2,1]);
    nny=nny/Norm_;

    AA_LD=prepare_LD(N_occu,iPEPS_2x2);#e0
...
function compute_ob_2x2_left_top(mps_bot_set,mps_top_set,iPEPS_2x2_, iPEPS_double_2x2, VL_set_set,VR_set_set, x_range,y_range)
    ...
    AA_LU=prepare_LU(N_occu,iPEPS_2x2);#e0
...
function compute_ob_2x2_triangle_new(mps_bot_set,mps_top_set,iPEPS_2x2_, iPEPS_double_2x2, VL_set_set,VR_set_set, x_range,y_range,Lx,Ly)
    ...
    AA_LD=remove_trivial_boundary(AA_LD,x_range[1],y_range[1],Lx,Ly);
    AA_RD=remove_trivial_boundary(AA_RD,x_range[2],y_range[1],Lx,Ly);
    nnx=build_fermi_cluster_2x2(mps_bot_set,mps_top_set,iPEPS_double_2x2[1,2],iPEPS_double_2x2[2,2],AA_LD,AA_RD, VL_set_set,VR_set_set, x_range,y_range,Lx,Ly);
    nnx=nnx/Norm_;
    ...
    AA_RU=remove_trivial_boundary(AA_RU,x_range[2],y_range[2],Lx,Ly);
    AA_RD=remove_trivial_boundary(AA_RD,x_range[2],y_range[1],Lx,Ly);
    nny=build_fermi_cluster_2x2(mps_bot_set,mps_top_set,iPEPS_double_2x2[1,2],AA_RU,iPEPS_double_2x2[1,1],AA_RD, VL_set_set,VR_set_set, x_range,y_range,Lx,Ly);
    nny=nny/Norm_;
    ...
    AA_LD=remove_trivial_boundary(AA_LD,x_range[1],y_range[1],Lx,Ly);
    AA_RD=remove_trivial_boundary(AA_RD,x_range[2],y_range[1],Lx,Ly);
    nn_ld_ru=build_fermi_cluster_2x2(mps_bot_set,mps_top_set,iPEPS_double_2x2[1,2],AA_RU,AA_LD,AA_RD, VL_set_set,VR_set_set, x_range,y_range,Lx,Ly);
    nn_ld_ru=nn_ld_ru/Norm_;
...
function compute_ob_2x2_top_new(mps_bot_set,mps_top_set,iPEPS_2x2_, iPEPS_double_2x2, VL_set_set,VR_set_set, x_range,y_range,Lx,Ly)
    ...
    AA_LU=remove_trivial_boundary(AA_LU,x_range[1],y_range[2],Lx,Ly);
    AA_RU=remove_trivial_boundary(AA_RU,x_range[2],y_range[2],Lx,Ly);
    nnx=build_fermi_cluster_2x2(mps_bot_set,mps_top_set,AA_LU,AA_RU,iPEPS_double_2x2[1,1],iPEPS_double_2x2[2,1], VL_set_set,VR_set_set, x_range,y_range,Lx,Ly);
    nnx=nnx/Norm_;
...
function compute_ob_2x2_left_new(mps_bot_set,mps_top_set,iPEPS_2x2_, iPEPS_double_2x2, VL_set_set,VR_set_set, x_range,y_range,Lx,Ly)
    ...
    AA_LU=remove_trivial_boundary(AA_LU,x_range[1],y_range[2],Lx,Ly);
    AA_LD=remove_trivial_boundary(AA_LD,x_range[1],y_range[1],Lx,Ly);
    nny=build_fermi_cluster_2x2(mps_bot_set,mps_top_set,AA_LU,iPEPS_double_2x2[2,2],AA_LD,iPEPS_double_2x2[2,1], VL_set_set,VR_set_set, x_range,y_range,Lx,Ly)
    nny=nny/Norm_;
