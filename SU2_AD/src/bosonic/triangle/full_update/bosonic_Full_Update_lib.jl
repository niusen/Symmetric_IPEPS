# Bosonic ALS kernels for the up-triangle iPESS full update.
#
# This is the sign-free counterpart of
# `src/fermionic/simple_update/Full_Update_lib.jl`. Tensor ordering is kept
# identical, while fermion-parity signs are replaced by ordinary bosonic
# permutations/identities.

function bosonic_permute_neighbour_ind(A, ind1::Integer, ind2::Integer, total_ind::Integer)
    @assert ind1 + 1 == ind2
    @assert total_ind == length(codomain(A)) + length(domain(A))
    order = collect(1:total_ind)
    order[ind1], order[ind2] = order[ind2], order[ind1]
    return permute(A, Tuple(order))
end

"""Identity sign tensor used where the fermionic code inserts a parity swap."""
function bosonic_sign_gate(V1, V2)
    return unitary(V1 ⊗ V2, V1 ⊗ V2)
end

bosonic_sign_gate(A, p1::Integer, p2::Integer) =
    bosonic_sign_gate(space(A, p1), space(A, p2))

function bosonic_check_positive(T::DiagonalTensorMap; warning_tol=1e-2)
    T_new = deepcopy(T)
    for cc in eachindex(T.data)
        if real(T.data[cc]) < 0
            T_new.data[cc] = zero(T.data[cc])
        end
    end
    denom = norm(T_new)
    relative_change = denom == 0 ? norm(T - T_new) : norm(T - T_new) / denom
    if relative_change > warning_tol
        @warn "Bosonic FU environment has a sizable negative component" relative_change
    end
    return T_new
end
function bosonic_split_3tensors(B1, B2, B3, T, op_LD_RD_RU)
    # """
    #          M1     R1
    #            \   /
    #             \ /....d1
    ...
    B1=bosonic_permute_neighbour_ind(B1,2,3,4);#M1, R1, d1,  D1
    ...
    B2=bosonic_permute_neighbour_ind(B2,3,4,4);#M2, d2, D2, R2
    B2=bosonic_permute_neighbour_ind(B2,2,3,4);#M2, D2, d2, R2
    uu,ss,vv=tsvd(permute(B2,(1,2,),(3,4,)));
    B2_res=uu;#M2, D2, new2
    B2_keep=ss*vv; #new2, d2, R2
    B2_res=bosonic_permute_neighbour_ind(B2_res,2,3,3);#M2, new2, D2
    ...
    B1_B2_T_B3=bosonic_build_triangle_from_4tensors(T,B1_keep,B2_keep,B3_keep);

    #d2',d3',d1', d2,d3,d1
    op_LD_RD_RU=bosonic_permute_neighbour_ind(op_LD_RD_RU,5,6,6);#d2',d3',d1', d2,d1,d3
    op_LD_RD_RU=bosonic_permute_neighbour_ind(op_LD_RD_RU,4,5,6);#d2',d3',d1', d1,d2,d3
    op_LD_RD_RU=bosonic_permute_neighbour_ind(op_LD_RD_RU,2,3,6);#d2',d1',d3', d1,d2,d3
    op_LD_RD_RU=bosonic_permute_neighbour_ind(op_LD_RD_RU,1,2,6);#d1',d2',d3', d1,d2,d3
    @tensor op_LD_RD_RU[:]:=bosonic_fu_Up[-1,1,2,3]*op_LD_RD_RU[1,2,3,4,5,6]*bosonic_fu_Up'[4,5,6,-2];
...
function bosonic_partial_triangle_partial_B1(Big_triangle,env_bot, T,B1_keep,B2_keep,B3_keep)
    #B1_keep: (new1, d1),  (D1)
    #Big_triangle: (new2, new1), (d123, new3)
    # gate1=bosonic_sign_gate(space(B2_keep,2),space(T,2));
    # gate1=permute(gate1,(2,1,),(3,4,));
    # gate2=bosonic_sign_gate(space(B2_keep,1),space(T,2));
    # gate2=permute(gate2,(2,1,),(3,4,));
    gate3=bosonic_sign_gate(space(B1_keep,2),space(B2_keep,1));
    gate3=permute(gate3,(2,1,),(3,4,));
    # gate4=bosonic_sign_gate(space(B1_keep,1),space(B2_keep,1));
    ...
    #apply gate4
    env_bot_new=bosonic_permute_neighbour_ind(env_bot_new,3,4,4);#new_ind,new3,new1,new2
    ...
    @tensor Big_triangle[:]:=Big_triangle[-1,-2,1,-5]*bosonic_fu_Up'[-3,2,3,1]*UU[-4,2,3];#(new2, new1, d1, d2d3, new3)
...
function bosonic_partial_triangle_partial_B2(Big_triangle,env_bot, T,B1_keep,B2_keep,B3_keep)
    #B1_keep: (new1, d1), (D1)
    #B2_keep: (new2, d2), (R2)
    #Big_triangle: (new2, new1), (d123, new3)
    gate1=bosonic_sign_gate(space(B2_keep,2),space(T,2));
    ...
    @tensor Big_triangle[:]:=Big_triangle[1,2,3,-6]*U21[-1,1,2]*bosonic_fu_Up'[-3,-4,-5,3];#(new2new1,  d1,d2,d3, new3)
...
function bosonic_partial_triangle_partial_B3(Big_triangle,env_bot, T,B1_keep,B2_keep,B3_keep)
    @tensor B2_T[:]:=B2_keep[-1,-2,1]*T[1,-3,-4];     #(new2, d2, R2),  (R2, D1, M3) => (new2, d2, D1, M3)
    B2_T=bosonic_permute_neighbour_ind(B2_T,2,3,4);#(new2, D1, d2, M3)
    B2_T=bosonic_permute_neighbour_ind(B2_T,1,2,4);#(D1, new2, d2, M3)
    @tensor B1_B2_T[:]:=B1_keep[-1,-2,1]*B2_T[1,-3,-4,-5];#(new1, d1,  D1), (D1, new2, d2, M3) => (new1, d1, new2, d2, M3)

    B1_B2_T=bosonic_permute_neighbour_ind(B1_B2_T,2,3,5);# new1, new2, d1, d2, M3
    B1_B2_T=bosonic_permute_neighbour_ind(B1_B2_T,1,2,5);# new2, new1, d1, d2, M3

    #env_bot: new_ind,new2,new3,new1
    @tensor env_bot_B1_B2_T[:]:=env_bot[-1,2,-2,1]*B1_B2_T[2,1,-3,-4,-5];#(new_ind,new2,new3,new1), (new2, new1, d1, d2, M3) -> (new_ind, new3, d1, d2, M3)

    @tensor Big_triangle[:]:=Big_triangle[-1,-2,1,-6]*bosonic_fu_Up'[-3,-4,-5,1];#(new2,new1,  d1,d2,d3, new3)
...
function bosonic_partial_triangle_partial_T(Big_triangle,env_bot, T,B1_keep,B2_keep,B3_keep)
    #T:(R2, D1), (M3)
    #B1_keep: (new1, d1), (D1)
    #B2_keep: (new2, d2), (R2)
    #B3_keep: (M3, d3), (new3)
...
function bosonic_sweep_iteration(B1_B2_T_B3_op,env_bot, B_new,T1_new,T2_new,T3_new)
    # jldsave("test1.jld2";B1_B2_T_B3_op,env_bot, B_new,T1_new,T2_new,T3_new)
    ####################################
    T1_left,T1_right,T1_new=bosonic_partial_triangle_partial_B1(B1_B2_T_B3_op,env_bot, B_new,T1_new,T2_new,T3_new);
    T2_left,T2_right,T2_new=bosonic_partial_triangle_partial_B2(B1_B2_T_B3_op,env_bot, B_new,T1_new,T2_new,T3_new);
    T3_left,T3_right,T3_new=bosonic_partial_triangle_partial_B3(B1_B2_T_B3_op,env_bot, B_new,T1_new,T2_new,T3_new);
    B_left,B_right,B_new=bosonic_partial_triangle_partial_T(B1_B2_T_B3_op,env_bot, B_new,T1_new,T2_new,T3_new);
...
function bosonic_sweep_optimizations(n_sweep,B1_B2_T_B3_op,env_top,env_bot, B_new,T1_new,T2_new,T3_new)

    ov_history=zeros(n_sweep);
    for ci=1:n_sweep
        B_new,T1_new,T2_new,T3_new=bosonic_sweep_iteration(B1_B2_T_B3_op,env_bot, B_new,T1_new,T2_new,T3_new);
        big_T_compressed_opt=bosonic_build_triangle_from_4tensors(B_new,T1_new,T2_new,T3_new);

        ov12=bosonic_get_overlap_env(env_top,env_bot,big_T_compressed_opt',B1_B2_T_B3_op);
        ov11=bosonic_get_overlap_env(env_top,env_bot,B1_B2_T_B3_op',B1_B2_T_B3_op);
        ov22=bosonic_get_overlap_env(env_top,env_bot,big_T_compressed_opt',big_T_compressed_opt);
