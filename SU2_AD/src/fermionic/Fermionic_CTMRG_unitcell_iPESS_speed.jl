...
function CTM_ite_cell_continuous_update(Cset_cell, Tset_cell, double_B_cell,double_T_cell, chi, direction, trun_tol,CTM_ite_info,projector_strategy,CTM_trun_svd,svd_lanczos_tol,construct_double_layer)
    ...
    for cx=1:cx_max
        ...
        for cy=1:cy_max
            ...
            println("test rand svd");flush(stdout);
            @time begin
            @sync @distributed for ccc =1:4 
                BLAS.set_num_threads(6);
                uu,ss,vv = svd(rand(100,100));
                end
            end

            println("test distributed1");flush(stdout);
            @time begin
            @sync @distributed   for ccc=1:4
                    BLAS.set_num_threads(6);
                    a=randn(100,100);
            ...
            println("test distributed 1.5");flush(stdout);
            @time begin
                parall_data=[];
                @sync @distributed    for ccc=1:4
                    BLAS.set_num_threads(6);
                    a=randn(100,100)*direction;
                end
            end

            println("test distributed2");flush(stdout);
            @time begin
                parall_data=[];
            ...
            println("build corner");flush(stdout);
            @time begin
                parall_data=[];
                parall_data=@sync @distributed  (append_data)  for ccc=1:4
        ...
        println("time of block svd");flush(stdout);
        @time begin
        ...
        for cy=1:cy_max
            ...
            println("time of building projected corners");flush(stdout);
            @time begin
        ...
        for cy=1:cy_max
            ...
            println("time of save corner matrix");flush(stdout);
            @time begin
            Pos=convert_cell_posit(coord[1],coord[2],1,0,direction);
