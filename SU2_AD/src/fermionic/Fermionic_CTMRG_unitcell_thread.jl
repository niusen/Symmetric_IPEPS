...
function get_MMup(coord,direction,AA_cell,Cset_cell,Tset_cell)
    ...
    @tensor MMup[:]:=C1[1,2]*T1[2,3,-3]*T4[-1,4,1]*AA_cell[Pos[1]][Pos[2]][4,-2,-4,3];
    return MMup
end
function get_MMlow(coord,direction,AA_cell,Cset_cell,Tset_cell)
    ...
    @tensor MMlow[:]:=T4[1,3,-1]*AA_cell[Pos[1]][Pos[2]][3,4,-4,-2]*C4[2,1]*T3[-3,4,2];
    return MMlow
end

function get_MMup_reflect(coord,direction,AA_cell,Cset_cell,Tset_cell)
    ...
    @tensor MMup_reflect[:]:=T1[-1,3,1]* C2[1,2]* AA_cell[Pos[1]][Pos[2]][-2,-4,4,3]* T2[2,4,-3];
    return MMup_reflect
end

function get_MMlow_reflect(coord,direction,AA_cell,Cset_cell,Tset_cell)
    ...
    @tensor MMlow_reflect[:]:=MMlow_reflect[-1,1,2,-3]*AA_cell[Pos[1]][Pos[2]][-2,1,2,-4];
    return MMlow_reflect
end
    ...
    for cx=1:cx_max
        ...
        @time begin
            if projector_strategy=="4x4"
                Threads.@threads for cm=1:4
                    if cm==1
                        MMup=get_MMup(coord,direction,AA_cell,Cset_cell,Tset_cell);
                    elseif cm==2
                        MMlow=get_MMlow(coord,direction,AA_cell,Cset_cell,Tset_cell);
                    elseif cm==3
                        MMup_reflect=get_MMup_reflect(coord,direction,AA_cell,Cset_cell,Tset_cell);
                    elseif cm==4
                        MMlow_reflect=get_MMlow_reflect(coord,direction,AA_cell,Cset_cell,Tset_cell);
                    end
                end
        ...
        println("bbbbbb2_check");flush(stdout);
        @time begin
            if CTM_trun_svd
                svd_err=norm(uM*sM*vM-M_);
                if svd_err>1e-6
                    @warn "error of truncated svd="*string(norm(uM*sM*vM-M_));
                end
            end
        end


        println("bbbbbb3");flush(stdout);
        @time begin
            sM_norm=norm(sM);
            sM=sM/sM_norm;
