...
function delet_zero_block(U,Σ,V)
    if isa(space(Σ,1),ComplexSpace)
        ...
        return convert(TensorMap, U_dict), TensorMap_to_DiagonalTensorMap(convert(TensorMap, Σ_dict)), convert(TensorMap, V_dict)
    end
end
...
function gate_RU_LD_RD_Hofstadter(energy_setting,parameters,dt, space_type,Lx,Ly)
    ...
    elseif space_type==GradedSpace{Z2Irrep, Tuple{Int64, Int64}}
        if (energy_setting.model == "Triangle_Hofstadter_Hubbard")|(energy_setting.model == "spinful_triangle_lattice")
            Ident_set, N_occu_set, n_double_set, Cdag_set, C_set=special_Hamiltonians_spinful_Z2();
            sx_op,sy_op,sz_op=spin_operator_Z2();
            Cdagup, Cup, Cdagdn,Cdn=special_Hamiltonians_spinful_Z2_split();
        elseif (energy_setting.model == "Triangle_Hofstadter_spinless")
            Ident_set, N_occu_set, n_double_set, Cdag_set, C_set=Hamiltonians_spinless_Z2();
        else
            error("unknown case")
    ...
    for cx=1:Lx
        for cy=1:Ly
            ####################
            @tensor op_up[:]:=Cdagup[1,-1,-3]*Cup[1,-2,-4];
            @tensor op_dn[:]:=Cdagdn[1,-1,-3]*Cdn[1,-2,-4];
            op=permute(op_up*tx_coe_set[cx,cy]+op_dn*tx_coe_set[cx,cy],(1,2,),(3,4,));
            hh=op+op';
            Id=unitary(space(Cdagup,2),space(Cdagup,2));
            @tensor hh_tx[:]:=hh[-1,-2,-4,-5]*Id[-3,-6];
            ######################
            @tensor op_up[:]:=Cdagup[1,-1,-3]*Cup[1,-2,-4];
            @tensor op_dn[:]:=Cdagdn[1,-1,-3]*Cdn[1,-2,-4];
            op=permute(op_up*ty_coe_set[cx,cy]'+op_dn*ty_coe_set[cx,cy]',(1,2,),(3,4,));
            hh=op+op';
            Id=unitary(space(Cdagup,2),space(Cup,2));
            @tensor hh_ty[:]:=hh[-2,-3,-5,-6]*Id[-1,-4];
            #####################
            @tensor op_up[:]:=Cdagup[1,-1,-3]*Cup[1,-2,-4];
            @tensor op_dn[:]:=Cdagdn[1,-1,-3]*Cdn[1,-2,-4];
            op_up=-op_up;#!!!!!!! somehow this minus sign is required
            op_dn=-op_dn;#!!!!!!! somehow this minus sign is required
            op=op_up*t2_coe_set[cx,cy]+op_dn*t2_coe_set[cx,cy];
            op=permute(op,(1,2,),(3,4,));
            hh=op+op';
            Id=unitary(space(Cdagup,2),space(Cdagup,2));
            @tensor hh[:]:=hh[-1,-3,-4,-6]*Id[-2,-5];
...
function gate_RU_LD_RD_Hofstadter_spinHall(energy_setting,parameters,dt, space_type,Lx,Ly)
    ...
    tx_up_coe_set=parameters_site["tx_up_coe_set"]/2;
    ty_up_coe_set=parameters_site["ty_up_coe_set"]/2;
    t2_up_coe_set=parameters_site["t2_up_coe_set"]/2;
    tx_dn_coe_set=parameters_site["tx_dn_coe_set"]/2;
    ty_dn_coe_set=parameters_site["ty_dn_coe_set"]/2;
    ...
    for cx=1:Lx
        for cy=1:Ly
            ...
            @tensor op_up[:]:=Cdagup[1,-1,-3]*Cup[1,-2,-4];
            @tensor op_dn[:]:=Cdagdn[1,-1,-3]*Cdn[1,-2,-4];
            op=permute(op_up*tx_up_coe_set[cx,cy]+op_dn*tx_dn_coe_set[cx,cy],(1,2,),(3,4,));
            hh=op+op';
            Id=unitary(space(Cdagup,2),space(Cdagup,2));
            @tensor hh_tx[:]:=hh[-1,-2,-4,-5]*Id[-3,-6];
            ######################
            @tensor op_up[:]:=Cdagup[1,-1,-3]*Cup[1,-2,-4];
            @tensor op_dn[:]:=Cdagdn[1,-1,-3]*Cdn[1,-2,-4];
            op=permute(op_up*ty_up_coe_set[cx,cy]'+op_dn*ty_dn_coe_set[cx,cy]',(1,2,),(3,4,));
            hh=op+op';
            Id=unitary(space(Cdagup,2),space(Cup,2));
            @tensor hh_ty[:]:=hh[-2,-3,-5,-6]*Id[-1,-4];
            #####################
            ...
            op_up=-op_up;#!!!!!!! somehow this minus sign is required
            op_dn=-op_dn;#!!!!!!! somehow this minus sign is required
            op=op_up*t2_up_coe_set[cx,cy]+op_dn*t2_dn_coe_set[cx,cy];
            op=permute(op,(1,2,),(3,4,));
            hh=op+op';
            Id=unitary(space(Cdagup,2),space(Cdagup,2));
            @tensor hh[:]:=hh[-1,-3,-4,-6]*Id[-2,-5];
