function initial_SU2_fPEPS_state(Vspace,init_statenm="nothing",init_noise=0,init_complex_tensor=false)
...
function cost_fun(x) #variational parameters are vector of TensorMap
    ...
    if energy_setting.model=="triangle_J1_J2_Jchi"
        ...
        E_T1, E_T2, E_T3, E_T4=evaluate_triangle_fpeps(H_triangle, A, AA, U_L,U_D,U_R,U_U, CTM, grad_ctm_setting);
    end
...
function energy_CTM(x, chi, parameters, ctm_setting, energy_setting, init, init_CTM)
    ...
    if energy_setting.model=="triangle_J1_J2_Jchi"
        ...
        E_T1, E_T2, E_T3, E_T4=evaluate_triangle_fpeps(H_triangle, A, AA, U_L,U_D,U_R,U_U, CTM, ctm_setting);
        E=real(E_T1+E_T2+E_T3+E_T4);
