...
function Gutzwiller_U1_SU2_2site(coe)
    ...
    elseif M==2
        U_phy1=unitary(Rep[U₁ × SU₂]((6, 0)=>1, (8, 0)=>3, (10, 0)=>1, (7, 1/2)=>2, (9, 1/2)=>2, (8, 1)=>1) ← (Rep[U₁ × SU₂]((10, 0)=>1) ⊗ Rep[U₁ × SU₂]((0, 0)=>1, (2, 0)=>1, (1, 1/2)=>1)' ⊗ Rep[U₁ × SU₂]((0, 0)=>1, (2, 0)=>1, (1, 1/2)=>1)'));
        U_phy2=unitary(Rep[U₁ × SU₂]((0, 0)=>3, (2, 0)=>1, (-2, 0)=>1, (1, 1/2)=>2, (-1, 1/2)=>2, (0, 1)=>1)  ← (Rep[U₁ × SU₂]((6, 0)=>1, (8, 0)=>3, (10, 0)=>1, (7, 1/2)=>2, (9, 1/2)=>2, (8, 1)=>1) ⊗ Rep[U₁ × SU₂]((-2, 0)=>1) ⊗ Rep[U₁ × SU₂]((-2, 0)=>1) ⊗ Rep[U₁ × SU₂]((-2, 0)=>1) ⊗ Rep[U₁ × SU₂]((-2, 0)=>1)));
        Vdummy=Rep[U₁ × SU₂]((1,1/2)=>1)';
        V=Rep[U₁ × SU₂]((0, 0)=>1, (2, 0)=>1, (1, 1/2)=>1)';
    end
    ...
    @tensor PG1[:]:=PG[-1,-3]*Ident2[-2,-4];
    PG1=TensorMap(PG1,  V*V  ←  V*V);
    @tensor PG2[:]:=Ident2[-1,-3]*PG[-2,-4];
    PG2=TensorMap(PG2,  V*V  ←  V*V);

    ###############################

    if M==1
        @tensor PG1[:]:=PG1[1,2,4,5]*U_phy1[-1,3,1,2]*U_phy1'[3,4,5,-2];
        @tensor PG1[:]:=PG1[3,4]*U_phy2[-1,3,1,2]*U_phy2'[4,1,2,-2];

        @tensor PG2[:]:=PG2[1,2,4,5]*U_phy1[-1,3,1,2]*U_phy1'[3,4,5,-2];
        @tensor PG2[:]:=PG2[3,4]*U_phy2[-1,3,1,2]*U_phy2'[4,1,2,-2];


    elseif M==2
        @tensor PG1[:]:=PG1[1,2,4,5]*U_phy1[-1,3,1,2]*U_phy1'[3,4,5,-2];
        @tensor PG1[:]:=PG1[3,4]*U_phy2[-1,3,1,2,5,6]*U_phy2'[4,1,2,5,6,-2];

        @tensor PG2[:]:=PG2[1,2,4,5]*U_phy1[-1,3,1,2]*U_phy1'[3,4,5,-2];
        @tensor PG2[:]:=PG2[3,4]*U_phy2[-1,3,1,2,5,6]*U_phy2'[4,1,2,5,6,-2];
...
function Hamiltonians_spinful_U1_SU2_2site(M)
    ...
    elseif M==2
        # space(U_phy1)
        # Rep[U₁ × SU₂]((6, 0)=>1, (8, 0)=>3, (10, 0)=>1, (7, 1/2)=>2, (9, 1/2)=>2, (8, 1)=>1) ← (Rep[U₁ × SU₂]((10, 0)=>1) ⊗ Rep[U₁ × SU₂]((0, 0)=>1, (2, 0)=>1, (1, 1/2)=>1)' ⊗ Rep[U₁ × SU₂]((0, 0)=>1, (2, 0)=>1, (1, 1/2)=>1)')
        # space(U_phy_dummy)
        # Rep[U₁ × SU₂]((6, 0)=>1, (8, 0)=>3, (10, 0)=>1, (7, 1/2)=>2, (9, 1/2)=>2, (8, 1)=>1) ← (Rep[U₁ × SU₂]((6, 0)=>1, (8, 0)=>3, (10, 0)=>1, (7, 1/2)=>2, (9, 1/2)=>2, (8, 1)=>1) ⊗ Rep[U₁ × SU₂]((0, 0)=>1))
        # space(U_phy2)
        # Rep[U₁ × SU₂]((0, 0)=>3, (2, 0)=>1, (-2, 0)=>1, (1, 1/2)=>2, (-1, 1/2)=>2, (0, 1)=>1) ← (Rep[U₁ × SU₂]((6, 0)=>1, (8, 0)=>3, (10, 0)=>1, (7, 1/2)=>2, (9, 1/2)=>2, (8, 1)=>1) ⊗ Rep[U₁ × SU₂]((-2, 0)=>1) ⊗ Rep[U₁ × SU₂]((-2, 0)=>1) ⊗ Rep[U₁ × SU₂]((-2, 0)=>1) ⊗ Rep[U₁ × SU₂]((-2, 0)=>1))
    ...
    elseif M==2
        @tensor Ident4[:]:=Ident4[1,2,4,5]*U_phy1[-1,3,1,2]*U_phy1'[3,4,5,-2];
        @tensor Ident4[:]:=Ident4[3,4]*U_phy2[-1,3,1,2,5,6]*U_phy2'[4,1,2,5,6,-2];
        ...
        @tensor n_double_A[:]:=n_double_A[1,2,4,5]*U_phy1[-1,3,1,2]*U_phy1'[3,4,5,-2];
        @tensor n_double_A[:]:=n_double_A[3,4]*U_phy2[-1,3,1,2,5,6]*U_phy2'[4,1,2,5,6,-2];

        @tensor n_double_B[:]:=n_double_B[1,2,4,5]*U_phy1[-1,3,1,2]*U_phy1'[3,4,5,-2];
        @tensor n_double_B[:]:=n_double_B[3,4]*U_phy2[-1,3,1,2,5,6]*U_phy2'[4,1,2,5,6,-2];

        @tensor Cdag_A[:]:=Cdag_A[1,2,4,5,-3]*U_phy1[-1,3,1,2]*U_phy1'[3,4,5,-2];
        @tensor Cdag_A[:]:=Cdag_A[3,4,-3]*U_phy2[-1,3,1,2,5,6]*U_phy2'[4,1,2,5,6,-2];
        Cdag_A=permute(Cdag_A,(3,1,2,));

        @tensor Cdag_B[:]:=Cdag_B[1,2,4,5,-3]*U_phy1[-1,3,1,2]*U_phy1'[3,4,5,-2];
        @tensor Cdag_B[:]:=Cdag_B[3,4,-3]*U_phy2[-1,3,1,2,5,6]*U_phy2'[4,1,2,5,6,-2];
        Cdag_B=permute(Cdag_B,(3,1,2,));

        @tensor C_A[:]:=C_A[-1,1,2,4,5]*U_phy1[-2,3,1,2]*U_phy1'[3,4,5,-3];
        @tensor C_A[:]:=C_A[-1,3,4]*U_phy2[-2,3,1,2,5,6]*U_phy2'[4,1,2,5,6,-3];

        @tensor C_B[:]:=C_B[-1,1,2,4,5]*U_phy1[-2,3,1,2]*U_phy1'[3,4,5,-3];
        @tensor C_B[:]:=C_B[-1,3,4]*U_phy2[-2,3,1,2,5,6]*U_phy2'[4,1,2,5,6,-3];
