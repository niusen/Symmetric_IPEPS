...
function evaluate_ob_cell_speed(parameters, A_cell::Tuple, AA_cell, CTM_cell, ctm_setting, energy_setting)
    """change of coordinate 
    (1,1)  (2,1)
    (1,2)  (2,2)
    ...
    elseif energy_setting.model=="spinful_triangle_lattice"
        ...
        elseif (Lx==2) & (Ly==2) 
            ...
            cx=1;cy=1;
            println("tx term");
            @time ex=hopping_x(CTM_cell,Cdag_set[mod1(cx+1,Lx)],C_set[mod1(cx+2,Lx)],A_cell,AA_cell,cx,cy,ctm_setting);
            println("ty term");
            @time ey=hopping_y(CTM_cell,Cdag_set[mod1(cx+2,Lx)],C_set[mod1(cx+2,Lx)],A_cell,AA_cell,cx,cy,ctm_setting);
            println("t2 term");
            @time e_diagonala=hopping_diagonala(CTM_cell,Cdag_set[mod1(cx+1,Lx)],C_set[mod1(cx+2,Lx)],A_cell,AA_cell,cx,cy,ctm_setting);
            println("onsite term");
            @time e0=ob_onsite(CTM_cell,N_occu_set[mod1(cx+1,Lx)],A_cell,AA_cell,cx,cy,ctm_setting);
            println("U term");
            @time eU=ob_onsite(CTM_cell,n_double_set[mod1(cx+1,Lx)]-(1/2)*N_occu_set[mod1(cx+1,Lx)]+(1/4)*Ident_set[mod1(cx+1,Lx)],A_cell,AA_cell,cx,cy,ctm_setting);
            @ignore_derivatives ex_set[cx,cy]=ex;
            @ignore_derivatives ey_set[cx,cy]=ey;
            @ignore_derivatives e_diagonala_set[cx,cy]=e_diagonala;
            @ignore_derivatives e0_set[cx,cy]=e0;
...
# function hopping_diagonala_speed(CTM,O1,O2,A_cell,AA_cell,cx,cy,ctm_setting)
#     global Lx,Ly
#     pos_LU=[mod1(cx+1,Lx),mod1(cy+1,Ly)];
#     pos_RU=[mod1(cx+2,Lx),mod1(cy+1,Ly)];
#     pos_LD=[mod1(cx+1,Lx),mod1(cy+2,Ly)];
...
#     if ctm_setting.grad_checkpoint
#         AA_LD_double,_,_,_,_=Zygote.checkpointed(build_double_layer_swap_speed, A_cell[pos_LD[1]][pos_LD[2]]',A_LD);
#         AA_RU_double,_,_,_,_=Zygote.checkpointed(build_double_layer_swap_speed, A_cell[pos_RU[1]][pos_RU[2]]',A_RU);
#         AA_RD_double,_,_,_,_=Zygote.checkpointed(build_double_layer_swap_speed, A_cell[pos_RD[1]][pos_RD[2]]',A_RD);
#     else
#         AA_LD_double,_,_,_,_=build_double_layer_swap_speed(A_cell[pos_LD[1]][pos_LD[2]]',A_LD);
#         AA_RU_double,_,_,_,_=build_double_layer_swap_speed(A_cell[pos_RU[1]][pos_RU[2]]',A_RU);
#         AA_RD_double,_,_,_,_=build_double_layer_swap_speed(A_cell[pos_RD[1]][pos_RD[2]]',A_RD);
#     end
...
# function build_double_layer_swap_speed(Ap,A)
#     # println(space(Ap))
#     # println(space(A))
#     println("analyse build_double_layer_swap")
...
# @time begin
#     println("0000");flush(stdout);
#     AA_fused=double_LD*double_RU;
# end


# @time begin
#     println("1111");flush(stdout);
#     ##########################
