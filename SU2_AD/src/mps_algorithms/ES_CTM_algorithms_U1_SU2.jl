...
function ES_CTMRG_ED_Kprojector(CTM,U_L,U_D,U_R,U_U,M,chi,N,EH_n,decomp=false,y_anti_pbc=false)
    ...
    if y_anti_pbc
        ...
        gate_O1=unitary(space(O1,3),space(O1,3));
        gate_O2=unitary(space(O2,3),space(O2,3));
    end
...
function k_projection(v_unprojected,N,kphase,normalize)
    ...
    for cc=1:N-1
        v_unprojected=momentum_translate(v_unprojected,N);
...
function ES_CTMRG_ED(CTM,U_L,U_D,U_R,U_U,D,chi,N,EH_n,decomp=false,y_anti_pbc=false)
    k_phase=[];
    eu=[];
    Qn=[];
    Spin=[];
    ...
    if decomp
        if y_anti_pbc
            ES_filenm="ES_CTMite_APBC_decomp"*"_D"*string(D)*"_N"*string(N)*"_chi"*string(chi)*".mat";
        else
            ES_filenm="ES_CTMite_PBC_decomp"*"_D"*string(D)*"_N"*string(N)*"_chi"*string(chi)*".mat";
        end
    else
        if y_anti_pbc
            ES_filenm="ES_CTMite_APBC"*"_D"*string(D)*"_N"*string(N)*"_chi"*string(chi)*".mat";
        else
            ES_filenm="ES_CTMite_PBC"*"_D"*string(D)*"_N"*string(N)*"_chi"*string(chi)*".mat";
        end
    end
    matwrite(ES_filenm, Dict(
        "k_phase" => k_phase,
...
function momentum_translate(v,N)
    v=deepcopy(v);
    if N==3
