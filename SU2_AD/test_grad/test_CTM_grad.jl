...
optim_setting.init_statenm="stochastic_iPESS_LS_D_8_chi_80.jld2";#"SimpleUpdate_D_6.jld2";#"nothing";
...
function cost_fun(x::Matrix{T})  where T<:iPEPS_ansatz_immutable #variational parameters are vector of TensorMap
    ...
    A_cell_=@ignore_derivatives deepcopy(A_cell);
    AA_cell_=@ignore_derivatives deepcopy(AA_cell);
    E_total,  _=evaluate_ob_cell(parameters, A_cell_::Tuple, AA_cell_, CTM_cell, LS_ctm_setting, energy_setting);
    E=real(E_total);
...
∂E=gradient(x ->cost_fun(x), x)[1];
jldsave("test_grad.jld2";∂E)