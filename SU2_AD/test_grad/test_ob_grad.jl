...
function cost_fun(x::Matrix{T})  where T<:iPEPS_ansatz_immutable #variational parameters are vector of TensorMap
    ...
    @ignore_derivatives CTM_cell_=@ignore_derivatives deepcopy(CTM_cell);

    E_total,  _=evaluate_ob_cell(parameters, A_cell::Tuple, AA_cell, CTM_cell_, LS_ctm_setting, energy_setting);
    E=real(E_total);
