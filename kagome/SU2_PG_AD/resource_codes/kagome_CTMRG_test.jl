...
function CTMRG_test(A,chi,init,auxi_tensors,ctm_setting,optim_settings)
    ...
    for ci=1:CTM_ite_nums
        ...
        for direction in direction_order
            if optim_settings.grad_checkpoint #use checkpoint to save memory
                C1,C2,C3,C4, T1,T2,T3,T4= Zygote.checkpointed(CTM_ite_test, Cset, Tset, get_Tset(AA_rotated, direction), chi, direction,CTM_trun_tol,CTM_ite_info,projector_strategy,CTM_trun_svd,svd_lanczos_tol,construct_double_layer);
                Cset=Cset_struc(C1,C2,C3,C4);
                Tset=Tset_struc(T1,T2,T3,T4);
            else
                C1,C2,C3,C4, T1,T2,T3,T4=CTM_ite_test(Cset, Tset, get_Tset(AA_rotated, direction), chi, direction,CTM_trun_tol,CTM_ite_info,projector_strategy,CTM_trun_svd,svd_lanczos_tol,construct_double_layer);
                Cset=Cset_struc(C1,C2,C3,C4);
                Tset=Tset_struc(T1,T2,T3,T4);
            end
        end
...
function CTM_ite_test(Cset, Tset, AA, chi, direction, trun_tol,CTM_ite_info,projector_strategy,CTM_trun_svd,svd_lanczos_tol,construct_double_layer)
