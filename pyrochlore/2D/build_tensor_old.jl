function build_PEPS(D,coe,virtual_type,Irrep)
    ...
    elseif virtual_type=="square"
        if Irrep=="A1"
            A_set,A1_set,A2_set,B1_set,B2_set, S_label, Sz_label, virtual_particle, Va, Vb=construct_tensor(D,virtual_type);
            bond_tensor=A_set[1];
        ...
        elseif Irrep=="A1+iB1"
            ...
            bond_tensor=A_set[1];
            tetrahedral_tensora=A1_set[1];
            tetrahedral_tensorb=B1_set[1];
            tetrahedral=tetrahedral_tensora*coe[1]+tetrahedral_tensorb*coe[2]*im;
