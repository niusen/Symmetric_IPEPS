...
function build_density_op_cell(U_phy, A_unfused_cell, AA_cell, U_L_cell,U_D_cell,U_R_cell,U_U_cell, CTM, ca,cb,Lx,Ly)

    AA_open_1, U_s_s=build_double_layer_open(A_unfused_cell[mod1(ca+1,Lx),cb],"1",U_phy,U_L_cell[mod1(ca+1,Lx),cb],U_D_cell[mod1(ca+1,Lx),cb],U_R_cell[mod1(ca+1,Lx),cb],U_U_cell[mod1(ca+1,Lx),cb]);
    @tensor AA1[:]:=AA_open_1[-1,-2,-3,-4,1]*U_s_s[-5,-6,1];
    @tensor AA1[:]:=AA1[-1,-2,-3,-4,1,1];
    @assert norm(AA1-permute(AA_cell[mod1(ca+1,Lx),cb],(1,2,3,4,)))/norm(AA1)<1e-10

    AA_open, U_s_s_,fuse_spin=build_double_layer_open(A_unfused_cell[ca,cb],"12",U_phy,U_L_cell[ca,cb],U_D_cell[ca,cb],U_R_cell[ca,cb],U_U_cell[ca,cb]);
    @tensor AA_[:]:=AA_open[-1,-2,-3,-4,1]*U_s_s_[2,3,1]*fuse_spin[2,a,b]*fuse_spin'[a,b,3]
    @assert norm(AA_-permute(AA_cell[ca,cb],(1,2,3,4,)))/norm(AA_)<1e-10

    AA_open_2, _=build_double_layer_open(A_unfused_cell[ca, mod1(cb+1,Ly)],"2",U_phy,U_L_cell[ca, mod1(cb+1,Ly)],U_D_cell[ca, mod1(cb+1,Ly)],U_R_cell[ca, mod1(cb+1,Ly)],U_U_cell[ca, mod1(cb+1,Ly)]);

    AA_fused=AA_cell[mod1(ca+1,Lx),mod1(cb+1,Ly)];
    ...
    rho=density_op(CTM,AA_fused,AA_open,AA_open_1,AA_open_2, ca,cb,Lx,Ly);#L,U,D,R in a plaquatte
...
function density_op(CTM,AA_fused,AA_12,AA_1,AA_2, ca,cb,Lx,Ly)
    Cset=CTM["Cset"];
    Tset=CTM["Tset"];

    @tensor MM_LU[:]:=Cset[1][mod1(ca-1,Lx),mod1(cb+1,Ly)][1,2]*Tset[1][ca,mod1(cb+1,Ly)][2,3,-3]*Tset[4][mod1(ca-1,Lx),cb][-1,4,1]*AA_12[4,-2,-4,3,-5]; 
    @tensor MM_RU[:]:=Tset[1][mod1(ca+1,Lx),mod1(cb+1,Ly)][-1,3,1]* Cset[2][mod1(ca+2,Lx),mod1(cb+1,Ly)][1,2]* AA_1[-2,-4,4,3,-5]* Tset[2][mod1(ca+2,Lx),mod1(cb,Ly)][2,4,-3];

    @tensor MM_LD[:]:=Tset[4][mod1(ca-1,Lx),mod1(cb-1,Ly)][1,3,-2]*AA_2[3,4,-5,-3,-1]*Cset[4][mod1(ca-1,Lx),mod1(cb-2,Ly)][2,1]*Tset[3][mod1(ca,Lx),mod1(cb-2,Ly)][-4,4,2]; 
    @tensor MM_RD[:]:=Tset[2][mod1(ca+2,Lx),mod1(cb-1,Ly)][-4,-3,2]*Tset[3][mod1(ca+1,Lx),mod1(cb-2,Ly)][1,-2,-1]*Cset[3][mod1(ca+2,Lx),mod1(cb-2,Ly)][2,1]; 
