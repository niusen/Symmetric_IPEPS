...
function change_virtual(A_cell)
    ...
    U1=TensorMap(randn,V1,V);
    U1p=pinv(U1);
    U2=TensorMap(randn,V2,V);
    U2p=pinv(U2);
    U3=TensorMap(randn,V3,V);
    U3p=pinv(U3);
    ...
    @tensor A11[:]:=A11[-1,1,-3,-4,-5]*U1p[1,-2];
    @tensor A12[:]:=A12[-1,-2,-3,1,-5]*U1[-4,1];

    @tensor A21[:]:=A21[-1,1,-3,-4,-5]*U2p[1,-2];
    @tensor A22[:]:=A22[-1,-2,-3,1,-5]*U2[-4,1];

    @tensor A21[:]:=A21[-1,-2,-3,1,-5]*U3[-4,1];
    @tensor A22[:]:=A22[-1,1,-3,-4,-5]*U3p[1,-2];

    @tensor A11[:]:=A11[-1,-2,1,-4,-5]*U1p[1,-3];
    @tensor A21[:]:=A21[1,-2,-3,-4,-5]*U1[-1,1];

    @tensor A12[:]:=A12[-1,-2,1,-4,-5]*U2p[1,-3];
    @tensor A22[:]:=A22[1,-2,-3,-4,-5]*U2[-1,1];

    @tensor A12[:]:=A12[1,-2,-3,-4,-5]*U3[-1,1];
    @tensor A22[:]:=A22[-1,-2,1,-4,-5]*U3p[1,-3];
...
function build_double_layer(A,operator)
    ...
    #@assert norm(U_R-U_R0)/norm(U_R)<1e-10

    #U_U0=inv(U_D);
    U_U=unitary(space(A, 4) ⊗ space(A, 4)', fuse(space(A, 4)' ⊗ space(A, 4)));
    # println(U_U0)
    # println(U_U)
    # @assert norm(U_U-U_U0)/norm(U_U)<1e-10
